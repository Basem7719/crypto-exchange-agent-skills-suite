...
**Category:** strategy   |   **Mode:** analysis   |   **Exchanges:** Binance, OKX

> Design a rigorous backtest plan for a crypto trading strategy on Binance or OKX before any code is written. Takes a strategy description ("EMA cross with RSI filter", "funding-rate fade", "OI breakout"), produces a complete backtest specification: data sources, sample period, wal
...
```
/strategy-backtest-planner help
```
...
```
crypto-exchange-master  →  routes user requests  →  strategy-backtest-planner  →  hands off to next skill
```
